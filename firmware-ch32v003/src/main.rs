...
use keyer_core::{
    hal::{Duration, HalError, InputPaddle, Instant, OutputKey},
    Element, KeyerConfig, KeyerFSM, KeyerMode, PaddleInput, PaddleSide,
};
use riscv_rt::entry;
...
fn debug_heartbeat(last_heartbeat: &mut Instant) {
    ...
    if now_instant.duration_since(*last_heartbeat).as_millis() >= 10000 {
        info!(
            "💓 Heartbeat - Tx: {}, Queue: {}, Activity: {}ms ago",
